...
class UpdateUserDataTests(APITestCase):
    ...
    def test_update_data_with_negativ_train_exp(self):
    ...
    def test_update_data_with_negativ_age(self):
    ...
    def test_update_data_with_unexisting_gender(self):
        """Tests update user data with unexisting gender."""
        user = User.objects.get()
        response = login_user(self, login_data)
        user_auth_token = response.data['token']
        data = self.updating_data.copy()
