...
class ChallengeMemberService:
    ...
    @staticmethod
    def get_challenge_member(user: User, challenge: Challenge) -> Optional[ChallengeMember]:
    ...
    @staticmethod
    def has_user_already_accepted_this_challenge(user: User, challenge: Challenge) -> bool:
        """Checks has user already accepted this challenge"""
        challenge_member = ChallengeMember.objects.all().filter(user=user, challenge=challenge)
